...
export default function ArtDetailFromProfile({ route, navigation }) {
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            const endpoint = `${SERVER_BASE_URL}/api/v1/${flag}/${item._id}`;
            const headers = {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await fetch(
                `${SERVER_BASE_URL}/api/v1/${flag}/${item._id}`,
                {
                    method: "DELETE",
                }
            );
        ...
        <SafeAreaView style={styles.container}>
            <ScrollView>
                <Pressable onPress={pickImage}>
                    <Image
                        source={{ uri: editableItem.artwork }}
                        style={styles.image}
                    />
                </Pressable>
                ...
                {location && (
                    <>
                        <View style={styles.fieldContainer}>
                            ...
                            <Pressable
                                onPress={() => handleFieldEdit("message")}
                            >
                                <Feather name="edit" size={18} color="black" />
                            </Pressable>
                        </View>
